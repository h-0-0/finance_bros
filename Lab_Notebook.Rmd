...
```{r}
g_results <- glasso::glasso(covs, rho = 900000)
...
#M.1.1 <- fit_svm(D.1.tt[1]$train, gamma= T.1$best.parameters$gamma, C=T.1$best.parameters$cost, eps=T.1$best.parameters$epsilon, k_cross=10)
#summary(M.1)
...
```{r}
#pr <- SVMForecast::predict_svm(M.1, D.1[test_range,])
P.1 <- pred_multi(D.1, tt_ind.1, M.1)
for(i in P.1){print(i)}
```

```{r}
#plt <- plotf(D.1, tt_ind.1[[1]]$train, tt_ind.1[[1]]$test, P.1[[1]])
Plts.1 <- list()
for (i in 1:length(tt_ind.1)) {Plts.1[[i]] <- plotf(D.1, tt_ind.1[[i]]$train, tt_ind.1[[i]]$test, P.1[[i]]) }
cowplot::plot_grid(plotlist = Plts.1) 
```


#TODO: generate test and training range
#TODO: predict and return in desired format
#TODO: plot training data and predicted and target points
