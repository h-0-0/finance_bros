...
We will use the optimal hyperparameters we found during tuning and fit a model using only a portion of the data (the training data, in black on the figure below), we will then test our model on the remaining data (the testing data, in red in the figure below).
...
## Selecting features
We will start by selecting the sets of tickers we would like to use to compose our new feature spaces. 
#TODO: formulate subsets of tickers that you are going to use to fit svm's, with reasoning
  # Using multiple lags?
  # Volume?
  # Getting rid of weird tickers
