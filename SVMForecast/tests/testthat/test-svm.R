...
test_that("fit_svm works with setting params", {
  D <- import_stonks()
  out1 <- fit_svm(D, "BTC_USD~ DOGE_USD_1", degree=5, gamma= 1, C=2, eps=0.2, k_cross=10)
  out2 <- svm(BTC_USD~ DOGE_USD_1, D, type="eps-regression", kernel="radial", degree=5, gamma=1, cost=2, epsilon=0.2, cross=10)
  expect_equal(out1$formula, out2$formula)
  expect_equal(out1$type, out2$type)
  expect_equal(out1$kernel, out2$kernel)
  expect_equal(out1$degree, out2$degree)
