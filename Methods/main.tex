...
Minimising the objective function given above, can then be rewritten as a constrained optimisation problem by introducing slack variables $\xi$, $\hat{\xi}$. We also add the 

This is given by:


\begin{align}
\argmin_{\xi,\hat{\xi},\bw'} \; {\sum_{i = 1}^{n}}{(\xi_i + \hat{\xi_i})} + \frac{1}{2}||\bw'||^2 \\
